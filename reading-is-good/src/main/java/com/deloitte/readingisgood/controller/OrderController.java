...
public class OrderController {
    ...
    @GetMapping("/filter")
    public ResponseEntity<List<Order>> getOrdersByFilter(@RequestParam("from") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
                                                             @RequestParam("to") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to,
                                                             @RequestParam(value = "page", required = false, defaultValue = "0") Integer page,
                                                             @RequestParam(value = "size", required = false, defaultValue = "10") Integer size){
        LOG.info("get all books started");
        List<Order> response = orderService.getOrdersByFilter(from,to,page,size);
        LOG.info("get all books returned");
